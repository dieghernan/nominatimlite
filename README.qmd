...
generate synthetic addresses for OSM points (reverse geocoding).
...
You can also extract polygon and line objects when the Nominatim API provides
...
This example returns only latitude and longitude from the geocoder service. Use
`full_results = TRUE` to return all data from the geocoder service.
...
use `reverse_geo_lite()`. The arguments are similar to `geo_lite()`, but now we
specify the input data columns with the `lat` and `long` arguments. The dataset
used here is from the geocoder query above.
