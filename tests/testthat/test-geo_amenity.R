test_that("Progress bar", {
  local_mocked_bindings(
    geo_lite_struct = function(amenity, lat = "lat", long = "lon", ...) {
  ...
  # Not
  expect_silent(
    geo_amenity(bbox, c("pub", "restaurant"), progressbar = FALSE)
  )
})

test_that("geo_amenity() forwards bbox options and deduplicates amenities", {
  bbox <- c(-2, 40, -1, 41)
  state <- new.env(parent = emptyenv())
  state$calls <- list()

  local_mocked_bindings(
    geo_lite_struct = function(amenity,
                               lat,
                               long,
                               limit,
                               full_results,
...
test_that("geo_amenity() strict keeps only rows inside bbox", {
  local_mocked_bindings(
    geo_lite_struct = function(amenity, lat, long, ...) {
      dplyr::tibble(
        q_amenity = amenity,
        lat = c(0.5, 2),
        lon = c(0.5, 2),
