test_that("geo_lite_sf() returns an empty geometry when no address matches", {
...
test_that("geo_lite_sf() parses a successful GeoJSON response", {
...
test_that("geo_lite_sf() queries unique addresses and restores input order", {
  local_mocked_bindings(geo_lite_sf_single = function(address, ...) {
    mock_geo_sf(address)
  })
...
test_that("geo_lite_sf() displays progress only for multiple enabled queries", {
  local_mocked_bindings(geo_lite_sf_single = function(address, ...) {
    mock_geo_sf(address)
  })
  ...
  # Not
  expect_silent(geo_lite_sf(c("Madrid", "Barcelona"), progressbar = FALSE))
})
test_that("geo_lite_sf() returns empty geometry when the API is unavailable", {
